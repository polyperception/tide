...
def print_table(rows: list, title: str = None, file=sys.stdout):
    ...
    if title:
        ...
        print(('{:>%ds}' % (left_pad + len(title))).format(title), file=file)

    print(thick_divider, file=file)
    for row in rows:
        # Print each row while padding to each column's text width
        print('  ' +
              '   '.join([('{:>%ds}' % col_widths[col_idx]).format(row[col_idx]) for col_idx in range(len(row))]) +
              '  ',
              file=file)
        if row == rows[0]:
...
class Plotter():
    ...
    def make_summary_plot(self, out_dir: str, errors: dict, model_name: str, rec_type: str, hbar_names: bool = False):
        ...
        patches, outer_text, inner_text = ax.pie(error_sizes,
        ...
        sns.barplot(data=error_dfs['main'], x='Delta mAP', y='Error Type', ax=ax, palette=self.colors_main.values())
        ...
        sns.barplot(data=error_dfs['special'],
                    x='Error Type',
                    y='Delta mAP',
                    ax=ax,
